...
5 8
もし花火の制限がなければ
普通にナップサックdp 3000 * 3000
打ち上げられるのがS
上限Sで組んだ場合 + 上限T - Sで組んだ場合
仕切りをどこにするか
SとT - Sの値だけ保持しとく
"""

# JOI11本戦　C - 夜店 (Night Market)
# 上の交換型
# 上限limitでi個目まで進めた時のvalueの最大値
# wantの部分だけしかいらないなら
def knapsack_1(N, limit, weight, value, want):
    prev = [0] * (limit + 1)
    res = [0] * (N + 1)
...
fo = knapsack_1(N, T, W, V, S)
ba = knapsack_1(N, T, W[::-1], V[::-1], T - S)

# 前半部分i個まで時間Sで見る + 後半N - i個を時間T - Sで見る
ans = 0
for i in range(N + 1):
    ans = max(ans, fo[i] + ba[N - i])
print(ans)
